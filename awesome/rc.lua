...
function colorify_range(text, mid_boundary, high_boundary)
  ...
  parsed_number = tonumber(string.match(text, "%d+%.?%d*"))

  if parsed_number then
    if parsed_number < mid_boundary then
      return markup.fg.color(beautiful.fg_ok, text)
    elseif parsed_number < high_boundary then
      return markup.fg.color(beautiful.fg_warn, text)
    else
      return markup.fg.color(beautiful.fg_error, text)
    end
  else
    return text
  end
...
dmenu_cmd = string.format(
  "dmenu_run -nb '%s' -nf '%s' -sb '%s' -sf '%s' -fn '%s'",
  beautiful.bg_normal,
  beautiful.fg_normal,
  beautiful.bg_focus,
  beautiful.fg_focus,
...
lain.widgets.contrib.redshift:attach(
  ...
  function()
    if lain.widgets.contrib.redshift:is_active() then
      redshiftwidget:set_markup(markup.fg.focus("RS"))
    else
      redshiftwidget:set_text("RS")
    end
  end
)
